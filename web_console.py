...
# RTT 轮询间隔。高频 RTT 输出时，过短的间隔会放大 Python、USB 和
# WebSocket 调度开销；日志通过下面的批处理机制保持较低延迟。
READ_INTERVAL = 0.005
RTT_SEARCH_START = None        # 留空 → J-Link 自动搜索 _SEGGER_RTT
RTT_SEARCH_RANGE = 0
FLASH_MAX_ATTEMPTS = 3         # 闪存操作中断性故障（halt/RAMCode 下载失败）的重试次数

# RTT 日志背压参数。日志洪峰时宁可丢弃较早的日志，也不能让 asyncio
# 协程、浏览器消息队列和 DOM 无限增长，最终拖垮整台机器。
LOG_BATCH_BYTES = 32 * 1024
LOG_BATCH_INTERVAL_S = 0.05
LOG_PENDING_MAX_BYTES = 512 * 1024
...
class WsHub:
    ...
    * broadcast(payload, session_id) 只发给该会话；
    ...
    def __init__(self):
        ...
        self.loop = None
        # 每个会话只允许一个日志发送协程。新的日志先合并到 pending_logs，
        # 避免每个 RTT 小片段都调用 run_coroutine_threadsafe，造成无界排队。
        self.pending_logs = {}    # session_id -> str
        self.log_send_scheduled = set()
        self.log_generation = {}  # session_id -> int，避免重连时旧协程干扰新连接

    def attach(self, ws, session_id=None):
        with self.lock:
            if session_id:
                self.sessions[session_id] = ws
                self.pending_logs.pop(session_id, None)
                self.log_generation[session_id] = self.log_generation.get(session_id, 0) + 1
                self.log_send_scheduled.discard(session_id)
        ...
        with self.lock:
            ...
            for sid, w in list(self.sessions.items()):
                if w is ws:
                    ...
                    self.pending_logs.pop(sid, None)
                    self.log_generation[sid] = self.log_generation.get(sid, 0) + 1
                    self.log_send_scheduled.discard(sid)
    ...
    def send_log(self, text, session_id=None):
        ...
        with self.lock:
            ...
            pending = self.pending_logs.get(session_id, '') + str(text)
            if len(pending) > LOG_PENDING_MAX_BYTES:
                notice = (
                    '\n[JLinkHub] RTT 日志过快，已丢弃较早日志，'
                    '当前仅保留最新内容。\n'
                )
                pending = notice + pending[-(LOG_PENDING_MAX_BYTES - len(notice)):]
            self.pending_logs[session_id] = pending

            if session_id in self.log_send_scheduled:
                return
            self.log_send_scheduled.add(session_id)
            generation = self.log_generation.get(session_id, 0)

        async def _drain():
            while True:
                with self.lock:
                    if generation != self.log_generation.get(session_id, 0):
                        return
                    current_ws = self.sessions.get(session_id)
                    current = self.pending_logs.pop(session_id, '')
                    if current_ws is None or not current:
                        self.log_send_scheduled.discard(session_id)
                        return
                try:
                    await current_ws.send(self._serialize({
                        "type": "log", "data": current
                    }))
                    # 给同一事件循环中的 HTTP/WS 任务让出执行机会，避免日志
                    # 洪峰时单个发送协程长期独占事件循环。
...
class JlinkManager:
    ...
    def _reader_loop(self):
        ...
        def flush_logs():
            nonlocal log_parts, log_bytes, log_session, last_log_flush
            if log_parts and log_session is not None:
                hub.send_log(''.join(log_parts), session_id=log_session)
            log_parts = []
            log_bytes = 0
            log_session = None
            last_log_flush = time.monotonic()

        def queue_log(text, session):
            nonlocal log_bytes, log_session
            if not text or session is None:
                return
            # 连接会话切换时，不能把前一个会话的日志带给后一个会话。
            if log_session is not None and log_session != session:
                flush_logs()
            log_session = session
        ...
        while self.reading:
            ...
            try:
                ...
                for chunk in chunks:
                    queue_log(self.converter.feed(chunk), session)
                out = self.converter.tick()
                queue_log(out, session)

                # 按大小或时间刷新：正常情况下延迟不超过 50ms，日志洪峰时
                # 单次消息不超过约 32KB，显著减少 WebSocket 调度次数。
                if (log_bytes >= LOG_BATCH_BYTES or
                        (log_parts and time.monotonic() - last_log_flush >= LOG_BATCH_INTERVAL_S)):
                    flush_logs()
            except Exception as e:
                log(f"读取异常: {e}")
